...
public class FileIndexTester extends AbstractTester {
	...
	public static void testFileIndex(){
		...
		if (fIndex.getAverageRankForWord("rocky") != Math.round((1+4+2)/3.)){
			System.out.println("fIndex.getAverageRankForWord(\"rocky\")= " + fIndex.getAverageRankForWord("rocky"));
			List<RankedWord> list = fIndex.getSortedListByType(rankType.average); 
			System.out.println(list.get(list.indexOf(fIndex.getRankedWord("rocky"))).toString());
			printErrorNum(9);
		}
